...
"""
    ...
    # Generate using AI - with optimized settings
    EMERGENT_KEY = os.environ.get("EMERGENT_LLM_KEY")
    
    try:
        # Use gpt-4o for faster response (still high quality)
        chat = LlmChat(
            api_key=EMERGENT_KEY,
        ...
        logger.warning(f"GPT-4o failed for elite+ report: {e}")
        try:
            # Fallback to gpt-4o-mini for even faster response
            chat = LlmChat(
                api_key=EMERGENT_KEY,
